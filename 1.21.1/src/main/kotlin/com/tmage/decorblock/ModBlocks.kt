...
object ModBlocks {
    ...
    val PLANK_SLABS = WOOD_TYPES.map { type -> registerSlab("planks_${type}_slab") { SlabBlock(WOOD_PROP) } }
    val STONE_SLABS = STONE_MATERIALS.map { type -> registerSlab("cobble_${type}_slab") { SlabBlock(STONE_PROP) } }
    val CONCRETE_SLABS = CONCRETE_TYPES.map { type -> registerSlab("concrete_${type}_slab") { SlabBlock(STONE_PROP) } }
    val VEINED_SLABS = VEINED_COLORS.map { color -> registerSlab("veined_${color}_slab") { SlabBlock(STONE_PROP) } }
    val TERRAZZO_SLABS = TERRAZZO_COLORS.map { color -> registerSlab("terrazzo_${color}_slab") { SlabBlock(STONE_PROP) } }

    private fun <T : Block> registerBlock(name: String, blockSupplier: Supplier<T>): DeferredHolder<Block, T> {
        val block = REGISTRY.register(name, blockSupplier)
        ITEM_REGISTRY.register(name, Supplier { BlockItem(block.get(), Item.Properties()) })
        return block
    }

    private fun <T : SlabBlock> registerSlab(name: String, blockSupplier: Supplier<T>): DeferredHolder<Block, T> {
        val block = REGISTRY.register(name, blockSupplier)
        ITEM_REGISTRY.register(name, Supplier { BlockItem(block.get(), Item.Properties()) })
        return block
    }
